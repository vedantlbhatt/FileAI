...
def load_models():
    ...
    try:
        ...
        print(f"[WARNING] YOLOv5 loading failed: {e}")
    print("[INFO] Loading CLIP models...")
    clip_proc = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")
    clip_mod = CLIPModel.from_pretrained("openai/clip-vit-base-patch32").to(device)
    clip_mod.eval()
    print("[INFO] Loading BLIP models...")
...
def parse_rtf_unrtf(file_path):
    ...
    if result.returncode == 0:
        ...
        print(f"[WARNING] UnRTF failed on {file_path}: {result.stderr}")
        return ""
...
def parse_txt(filepath: str) -> str:
    try:
        ...
        print(f"[WARNING] Skipping non-UTF8 or binary file: {filepath}")
        return ""
...
def parse_file(filepath: str) -> str:
    ...
    try:
        if ext == '.txt':
            ...
            print(f"[INFO] File extension {ext} not supported for parsing: {filepath}")
    except Exception as e:
        print(f"[ERROR] Error parsing file {filepath}: {e}")
...
def get_all_files_and_parse_optimized(directory: str,
    ...
    print(f"[INFO] Walking directory {directory} and parsing files...")
...
def load_embeddings_captions(save_dir):
    try:
        ...
        print(f"[WARN] Failed to load cached embeddings and metadata: {e}")
        return None, None, None, None, None


def embed_and_index_texts(texts: List[str]):
    print("[INFO] Embedding texts with SentenceTransformer...")
    ...
    index = faiss.IndexFlatL2(dim)
    index.add(embeddings_np)
    print("[INFO] Text embeddings indexed.")
...
def embed_images_with_object_detection(image_paths: List[str], yolo_model, clip_processor, clip_model, blip_processor, blip_model) -> Tuple[np.ndarray, List[Tuple[int, int]], List[str]]:
    ...
    for img_idx, img_path in enumerate(image_paths):
        try:
            ...
            print(f"[WARNING] Detection/embedding error for {img_path}: {e}")
            continue
...
def main():
    print("[START] Multimodal Semantic Search Pipeline")

    directory_path = "/Users/vedantbhatt/source"  # Change as needed
    query_text = "girls"  # Change query as needed

    start_time = time.time()

    yolo, clip_processor, clip_model, blip_processor, blip_model = load_models()

    all_texts, all_file_paths, image_paths = get_all_files_and_parse_optimized(directory_path)
    print(f"[INFO] Parsed {len(all_texts)} text files, {len(image_paths)} images found.")

    # Text embedding and indexing (no caching implemented here for brevity)
    text_index, text_model, _ = embed_and_index_texts(all_texts)

    # Try loading cached image embeddings, captions, and metadata
    ...
    # Detect which images are new
    set_cached_paths = set(image_paths_cached) if image_paths_cached else set()
    new_image_paths = [p for p in image_paths if p not in set_cached_paths]

    # Embed new images only
    if new_image_paths:
        print(f"[INFO] Processing {len(new_image_paths)} new images for embeddings...")
        embeddings_np_new, image_to_region_new, captions_new = embed_images_with_object_detection(new_image_paths, yolo, clip_processor, clip_model, blip_processor, blip_model)

        # Merge with cached
        if embeddings_np_cached is not None:
            embeddings_np = np.vstack([embeddings_np_cached, embeddings_np_new])
            image_to_region = image_to_region_cached + [(len(image_paths_cached or []) + idx[0], idx[1]) for idx in image_to_region_new]
            captions = captions_cached + captions_new
            image_paths_all = image_paths_cached + new_image_paths
        else:
            embeddings_np = embeddings_np_new
            image_to_region = image_to_region_new
            captions = captions_new
            image_paths_all = new_image_paths

        # Rebuild or update FAISS index
        image_index = build_image_index(embeddings_np)

        # Save updated cache
        save_embeddings_captions(embeddings_np, captions, image_to_region, image_paths_all, cache_path)
        print(f"[INFO] Updated image embeddings and metadata saved to cache.")
    else:
        print(f"[INFO] Using cached image embeddings for all {len(image_paths_cached)} images.")
    ...
    if embeddings_np is None or embeddings_np.size == 0:
        print("[WARN] No image embeddings available; skipping image search.")
    else:
        print(f"[INFO] Searching texts for query: '{query_text}'")
        text_results = hybrid_semantic_search(text_index, text_model, all_texts, all_file_paths, query_text, k=5, alpha=0.7)

        print(f"[INFO] Searching images for query: '{query_text}'")
        image_results = semantic_search_images(image_index, image_paths_all, image_to_region, captions, query_text, clip_processor, clip_model, k=10)

        print("\n[RESULTS] Text Search Results:")
        for res in text_results:
            print(f"File: {res['file_path']}")
            print(f" Score: {res['combined_score']:.4f} (Semantic: {res['semantic_score']:.4f}, Keyword: {res['keyword_score']:.4f})")
            print()

        print("\n[RESULTS] Image Search Results:")
        for res in image_results:
            print(f"File: {res['file_path']} (Region Index: {res['region_index']})")
    ...
    end_time = time.time()
    print(f"[END] Search completed in {end_time - start_time:.2f} seconds.")
